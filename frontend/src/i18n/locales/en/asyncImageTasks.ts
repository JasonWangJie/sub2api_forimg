export default {
  asyncImageTasks: {
    title: 'Async Image Tasks',
    description: 'Track image generation, billing, and object storage delivery',
    adminTitle: 'Async Image Task Center',
    adminDescription: 'Inspect and recover downstream image generation tasks across the site',
    summary: {
      active: '{count} active',
      completed: '{count} completed',
      attention: '{count} failed',
      successRate: '{rate} success rate',
      averageDuration: '{duration} average duration',
    },
    filters: {
      search: 'Task, model, prompt, or final account',
      searchPlaceholder: 'Search task ID, model, prompt, or final account',
      startDate: 'Submitted after',
      endDate: 'Submitted before',
      allStatuses: 'All statuses',
      allPlatforms: 'All platforms',
      allRequestTypes: 'All request types',
      allProviders: 'All storage providers',
    },
    columns: {
      taskId: 'Task ID',
      submittedAt: 'Task timing',
      platform: 'Platform',
      requestType: 'Request type',
      model: 'Model / Spec',
      results: 'Images / Storage',
      storage: 'Storage',
      owner: 'User',
      cost: 'Actual cost',
    },
    requestType: {
      text_to_image: 'Text to image',
      image_to_image: 'Image to image',
    },
    status: {
      queued: 'Queued',
      invoking: 'Calling upstream',
      upstream_succeeded: 'Upstream complete',
      uploading: 'Uploading',
      billing_pending: 'Billing pending',
      succeeded: 'Succeeded',
      failed: 'Failed',
      execution_unknown: 'Execution unknown',
      storage_failed: 'Storage failed',
      billing_failed: 'Billing failed',
      expired: 'Expired',
      upstream_image_url_fetch_retry: 'Reference image fetch timeout retry',
    },
    provider: {
      pending: 'Not stored',
      qiniu: 'Qiniu Cloud',
      aliyun: 'Alibaba Cloud OSS',
      tencent: 'Tencent Cloud COS',
      custom_s3: 'Custom S3',
    },
    empty: {
      title: 'No async image tasks',
      description: 'Tasks submitted through an enabled Gemini or OpenAI async image group will appear here.',
    },
    detail: {
      title: 'Image Task Details',
      submittedAt: 'Submitted',
      startedAt: 'Started',
      finishedAt: 'Finished',
      duration: 'Duration',
      actualCost: 'Actual cost',
      retryCount: 'Retries',
      referenceImages: 'Reference image URLs',
      referenceImage: 'Reference {index}',
      openReferenceImage: 'Open reference image {index} in a new window',
      request: 'Request and billing',
      protocol: 'Downstream protocol',
      requestedSize: 'Requested / actual size',
      aspectRatio: 'Aspect ratio',
      imageCount: 'Image count',
      billingStatus: 'Billing status',
      billingMode: 'Billing mode',
      upstreamRequestId: 'Upstream request ID',
      routing: 'Routing context',
      user: 'User',
      apiKey: 'API key',
      group: 'Group',
      account: 'Final account',
      accountAudit: 'Account attempts and reconciliation',
      accountAttemptCount: 'Attempted accounts',
      reconciliationStatus: 'Reconciliation status',
      lastFailureReason: 'Last failure reason',
      accountAttemptHistory: 'View attempt history ({count})',
      attemptedAt: 'Attempted at',
      attemptStatus: 'Attempt status',
      httpStatus: 'HTTP status',
      failureReason: 'Failure reason',
      error: 'Task error',
      results: 'Generated images',
      resultsPending: 'Images are not available yet.',
      resultAlt: 'Generated image {index}',
      timeline: 'Stage timeline',
    },
    accountAttemptStatus: {
      selected: 'Selected',
      succeeded: 'Succeeded',
      failed: 'Failed',
    },
    reconciliation: {
      none: 'Not required',
      pending: 'Pending',
      confirmed_success: 'Confirmed success',
      confirmed_failure: 'Confirmed failure',
      unavailable: 'Unavailable',
    },
    resume: {
      action: 'Resume post-processing',
      title: 'Resume task post-processing',
      message: 'Resume storage or billing for task {id}? The upstream image request will not be sent again.',
      success: 'Task post-processing resumed',
    },
    terminate: {
      action: 'End as failed',
      title: 'End task as failed',
      message: 'Mark task {id} as failed now? This will not retry image generation and cannot be undone.',
      success: 'Task ended as failed',
      currentPageAction: 'End current page ({count})',
      currentPageTitle: 'End current-page tasks',
      currentPageMessage: 'Mark {count} terminable tasks on this page as failed? Image generation will not be retried, and this cannot be undone.',
      currentPageConfirm: 'End {count} tasks',
      currentPageResult: 'Ended {terminated}, skipped {skipped}, failed {failed}',
    },
    errors: {
      load: 'Failed to load async image tasks',
      detail: 'Failed to load task details',
      resume: 'Failed to resume task post-processing',
      terminate: 'Failed to end task',
      batchTerminate: 'Failed to end current-page tasks',
      openResult: 'Failed to open the generated image',
    },
  },
}
