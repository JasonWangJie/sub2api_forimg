export default {
    ...
    backup: {
      ...
      imageStorage: {
        ...
        local: {
          ...
          publicBaseUrlPlaceholder: '例如 https://apiimg.example.com（你的站点域名，反代到本服务）',
          sitePublicBaseUrlHint: '签名图片用这个域名生成 /v1/images/local/...?exp=&sig=。请与网站一致（如 https://apiimg.tokensfree.xyz）。同时把下方「任务查询链接的站点公开地址」改成同一域名后保存。local_url 仅用于单独映射 data_dir 的静态图床域名，不要填站点首页。',
        ...
        maxInputGiBPerKey: '每个 API Key 活跃 SC 参考图容量（GiB）',
        taskRetentionDays: '任务记录保留天数',
        resultRetentionDays: '结果图片保留天数',
        advancedRuntime: '高级 Worker 与下载限制',
        workerLeaseSeconds: 'Worker 租约（秒）',
        recoveryIntervalSeconds: '恢复扫描间隔（秒）',
        retryBackoffSeconds: '存储/计费重试退避（秒）',
        ...
        referenceTransportLocalInlineData: '本地预下载并转 inlineData',
        referenceTransportFallbackInlineData: '先透传，抓取失败后回退 inlineData',
        openaiReferenceTransportHint: '混合模式只在明确抓图失败或上游要求 multipart 时切换，并会持久化本地阶段。',
        geminiReferenceTransportHint: '混合模式只在明确 URL 抓取失败时切换；普通 5xx 不改变传输方式。',
        referenceRetryTitle: '参考图抓取重试',
        upstreamRetryTitle: '上游临时失败重试',
        capacityRetryTitle: '账号与模型容量重试',
        retryMaxAttempts: '额外重试次数',
        retryBaseSeconds: '基础退避（秒）',
        retryMaxSeconds: '最大退避（秒）',
        totalMaxRetries: '单任务总重试上限',
        retryJitterPercent: '退避抖动（%）',
        retryAfterMaxSeconds: 'Retry-After 上限（秒）',
        storageRetryAttempts: '存储重试次数',
        billingRetryAttempts: '计费重试次数',
        downloadMaxBytes: '参考图最大字节数',
        downloadMaxPixels: '单张参考图最大像素',
        maxReferenceImages: '单任务参考图数量上限',
        maxReferenceTotalBytes: '单任务参考图总字节上限',
        maxReferenceTotalPixels: '单任务参考图总像素上限',
        downloadTimeoutSeconds: '参考图下载超时（秒）',
        referenceFetchConcurrency: '参考图下载并发闸门',
        referenceCacheTTLSeconds: '参考图缓存 TTL（秒）',
        referenceCacheMaxBytes: '参考图缓存容量（字节）',
        uploadTimeoutSeconds: 'SC 上传超时（秒）',
        downloadMaxRedirects: '参考图最大重定向次数',
		geminiHalfKModels: '允许 0.5K 的 Gemini 模型',
    ...
    users: {
      ...
      group: '分组',
      none: '无',
      geminiImageGroup: 'Gemini 生图组',
      openaiImageGroup: 'OpenAI 生图组',
      imagePlatformGroupsUpdated: '生图平台分组已更新',
      imagePlatformGroupsUpdateFailed: '更新生图平台分组失败',
    ...
    groups: {
      ...
      imagePricing: {
        ...
        allowAsyncImageGeneration: '开启下游异步生图接口',
        asyncImageGenerationHint: '仅新增异步接口使用该开关，上游调度与生图计费完全沿用当前分组规则。',
        ...
        notConfigured: '未配置',
        accountPoolsTitle: '生图账号池调度',
        accountPoolsHint:
          '选择一种生图路由维度。三套配置独立保存，切换模式不会清除其它模式中的账号。账号可写为 101, 102:1, 103:1；相同优先级继续按实时负载均衡。',
        poolModeLegend: '生图账号池调度模式',
        poolMode: {
          resolution: '按清晰度',
          model: '按生图模型',
          model_resolution: '模型 + 清晰度'
        },
        modelPlaceholder: '搜索候选模型或输入精确模型 ID',
        addModel: '添加模型',
        removeModel: '删除模型 {model}',
        modelAccounts: '调度账号',
        noModelsConfigured: '尚未添加模型；未配置的请求会使用分组默认账号池。',
        modelInvalidOrDuplicate: '模型 ID 无效、重复，或包含不支持的通配符。',
        poolLoadFailed: '生图账号池配置加载失败，本次保存不会覆盖现有账号池。',
        poolFallbackHint: '未配置对应池时回退分组默认账号池；对应池已配置但其中账号均不可调度时会直接失败，不会越池回退。模型 ID 按客户端请求值精确匹配并区分大小写。',
        pool1k: '1K 账号',
        pool2k: '2K 账号',
        pool4k: '4K 账号',
