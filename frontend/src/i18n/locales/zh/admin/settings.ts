export default {
    settings: {
      ...
      seo: {
        indexingTitle: '搜索引擎收录',
        indexingDescription: '开启后仅允许搜索引擎收录站点根首页，登录、后台、支付及其他页面仍会禁止收录。',
        indexingOn: '允许收录首页',
        indexingOff: '全站禁止收录',
        siteURL: '站点主 URL',
        siteURLPlaceholder: 'https://example.com/',
        siteURLHint: '用于 canonical、Open Graph、结构化数据和 sitemap，必须是无路径、查询参数或锚点的 HTTP(S) 根地址。',
        siteURLMissing: '尚未配置主 URL，canonical、结构化数据和 sitemap 暂不会输出。',
        siteURLInvalid: '请输入有效的 HTTP(S) 站点根地址，例如 https://example.com/。',
        httpsWarning: '生产站点建议使用 HTTPS，否则安全性和搜索引擎信任度可能受影响。',
        robotsOpen: '允许首页并阻止 API 路径',
        robotsClosed: '阻止所有搜索引擎抓取',
        openRobots: '打开 robots.txt',
        sitemapReady: '保存后可用',
        sitemapUnavailable: '需要开启收录并配置主 URL',
        openSitemap: '打开 sitemap.xml',
        metadataTitle: '首页元数据',
        metadataDescription: '这些内容会直接写入服务端返回的首屏 HTML，Google 和 Bing 无需等待 JavaScript 即可读取。',
        title: 'SEO 标题',
        titleHint: '建议 10–60 个字符；留空时使用“站点名称 - AI API Gateway”。',
        description: 'SEO 描述',
        descriptionHint: '建议 50–160 个字符；留空时使用通用设置中的站点副标题。',
        keywords: '关键词',
        keywordsPlaceholder: '输入后按回车',
        keywordsHint: '最多 20 个，可用逗号、分号或换行批量粘贴。此标签用于兼容部分搜索引擎，Google 基本不使用 meta keywords。',
        ...
        discoveryTitle: '分享图片与站长验证',
        discoveryDescription: '配置社交分享卡片，并绑定 Google Search Console、Bing Webmaster Tools 等站长平台。',
        socialImage: '社交分享图 URL',
        socialImagePlaceholder: 'https://example.com/social-card.jpg',
        socialImageHint: '可选。建议使用可公开访问的 HTTPS 图片，尺寸 1200×630；配置后输出 Open Graph 与 Twitter 大图卡片。',
        imageHTTPSWarning: '分享图片建议使用 HTTPS，确保搜索引擎和社交平台可以安全抓取。',
        verificationTags: '站长验证 Meta 标签',
        verificationPlaceholder: '<meta name="google-site-verification" content="verification-token" />',
        verificationHint: '每行粘贴一个包含 name 和 content 的 meta 标签。系统会拒绝脚本、事件属性、http-equiv、额外属性以及覆盖核心 SEO 标签的内容，并在保存时规范化输出。',
        previewTitle: '搜索结果预览',
        previewDescription: '根据当前表单实时展示首页可能呈现的标题、地址和描述。',
      ...
      imageConcurrency: {
        title: '生图并发闸门',
        description: '限制当前进程同时执行的生图请求；OpenAI 与 Gemini、同步与异步请求共享同一组槽位。',
        source: '当前来源',
        sourceSystem: '系统设置',
        sourceYaml: 'config.yaml',
        hotReloadHint: '保存后新进入的生图请求立即生效，无需重启；正在执行的请求不会被中断。',
        multiInstanceHint: '多实例部署时每个实例独立限制，总并发量约为实例数乘以这里的上限。',
        enabled: '启用进程级生图并发限制',
        enabledHint: '关闭后不执行这一层限制；普通文本模型请求始终不占用这些槽位。',
        maxConcurrentRequests: '同时处理上限',
        maxConcurrentRequestsHint: '0 表示不限制。',
        overflowMode: '达到上限后的处理方式',
        overflowWait: '进入等待队列',
        overflowReject: '立即返回 HTTP 429',
        waitTimeoutSeconds: '最长等待时间（秒）',
        waitTimeoutSecondsHint: '仅等待模式有效；0 表示不等待。',
        maxWaitingRequests: '等待队列上限',
        maxWaitingRequestsHint: '仅等待模式有效；0 表示不限制等待人数。',
        fallbackTitle: 'config.yaml 回退值',
        fallbackSummary: '启用：{enabled}，并发：{concurrency}，模式：{mode}，等待：{timeout} 秒，队列：{waiting}',
        yes: '是',
        no: '否',
        save: '保存并立即应用',
        reset: '恢复 config.yaml',
        resetConfirm: '确定删除系统设置覆盖并恢复 config.yaml 中的生图并发配置吗？',
        loadFailed: '读取生图并发设置失败',
        saveSuccess: '生图并发设置已保存并应用',
        saveFailed: '保存生图并发设置失败',
        resetSuccess: '已恢复 config.yaml 生图并发配置',
        resetFailed: '恢复 config.yaml 配置失败',
        invalidNonNegativeInteger: '并发数、等待时间和队列上限必须是非负整数'
      },
      gatewayForwarding: {
        title: '请求转发行为',
