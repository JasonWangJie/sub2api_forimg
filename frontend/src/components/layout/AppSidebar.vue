    ...
    h(
      ...
      [
        h('path', {
          'stroke-linecap': 'round',
          'stroke-linejoin': 'round',
          d: 'M9.53 16.122a3 3 0 00-5.78 1.128 2.25 2.25 0 01-2.4 2.245 4.5 4.5 0 008.4-2.245c0-.399-.078-.78-.22-1.128zm0 0a15.998 15.998 0 003.388-1.62m-5.043-.025a15.994 15.994 0 011.622-3.395m3.421 3.42a15.995 15.995 0 004.764-4.648l3.876-5.814a1.151 1.151 0 00-1.597-1.597L14.146 6.32a15.996 15.996 0 00-4.649 4.763m3.42 3.42a6.776 6.776 0 00-3.42-3.42'
        })
      ]
    )
}

const ImagePlazaIcon = {
  render: () =>
    h(
      'svg',
      { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': '1.5' },
...
//
// 条目顺序：密钥 → 用量 → 可用渠道 → 渠道状态 → 订阅/支付 → 兑换 → 安装与教程 → 资料。
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    { path: '/image-library', label: t('nav.imageLibrary'), icon: ImagePlazaIcon, hideInSimpleMode: true },
    ...
    {
      path: '/guides',
      label: t('nav.installAndGuides'),
      icon: BookIcon,
      expandOnly: true,
      children: [
        { path: '/guides/cc-switch', label: t('nav.guideCcSwitch'), icon: DocumentIcon },
        { path: '/guides/drawing', label: t('nav.guideDrawing'), icon: DocumentIcon },
        { path: '/guides/async-image-api', label: t('nav.guideAsyncImageApi'), icon: DocumentIcon },
        { path: '/guides/claude-code', label: t('nav.guideClaudeCode'), icon: DocumentIcon },
        { path: '/guides/codex', label: t('nav.guideCodex'), icon: DocumentIcon },
        { path: '/guides/basics', label: t('nav.guideBasics'), icon: DocumentIcon },
        { path: '/guides/docs', label: t('nav.guideDocs'), icon: DocumentIcon },
      ],
    },
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/image-moderation', label: t('nav.adminImageModeration'), icon: ImagePlazaIcon, hideInSimpleMode: true },
    {
      path: '/admin/channels',
