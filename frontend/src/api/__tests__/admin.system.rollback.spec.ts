...
describe('admin system rollback API', () => {
    ...
    const versions: RollbackVersionInfo[] = [
      {
        ...
        html_url: 'https://github.com/JasonWangJie/sub2api_forimg/releases/tag/v0.1.146'
      }
    ]
  ...
  it('rollback without a version posts no body (legacy backup rollback)', async () => {
    ...
    expect(post).toHaveBeenCalledWith('/admin/system/rollback', undefined, { timeout: 900_000 })
  })
})
