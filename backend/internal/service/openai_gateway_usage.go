...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	applyOpenAIForwardResultAccountMapping(ctx, result, account, accountMappingInputModel)
...
//nolint:unused // 保留旧测试和插件调用的简化返回约定。
...
func (s *OpenAIGatewayService) selectOpenAIVideoBillingModel(
	...
	if apiKeyHasConfiguredVideoPrice(apiKey, first, resolution) {
		return first
	}
	for _, candidate := range billingModels {
		candidate = strings.TrimSpace(candidate)
...
func (s *OpenAIGatewayService) calculateOpenAIImageCost(
	...
	if !allConfigured {
		refreshed := s.apiKeyWithFreshGroupMediaPricing(ctx, apiKey)
		if refreshed != apiKey {
			apiKey = refreshed
			groupConfig = imagePriceConfigFromAPIKey(apiKey)
		}
