...
const (
	...
	SettingKeyEmailFilter  = "email_filter"   // 禁止发送的收件邮箱，分号分隔
	...
	// SettingKeyImageConcurrencySettings stores the complete gateway image
	// concurrency policy as JSON. Absence means config.yaml remains authoritative.
	SettingKeyImageConcurrencySettings = "gateway_image_concurrency_settings"
	...
	SettingKeyUserUsageLatencyDivisor     = "user_usage_latency_divisor"    // 用户用量页延迟展示除数
...
const (
	UserUsageLatencyDivisorDefault = 1.0
	UserUsageLatencyDivisorMin     = 1.0
	UserUsageLatencyDivisorMax     = 1000.0
)
