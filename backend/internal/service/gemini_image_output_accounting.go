...
//     不构成回退。
type geminiObservedImageCounter struct {
	count int
}

// beginGeminiImageOutputObservation 在每次 Forward 开头重置计数器。
// failover 会拿同一个 gin.Context 重跑转发，不重置就会把上一个账号的图数带进来。
func beginGeminiImageOutputObservation(c *gin.Context) *geminiObservedImageCounter {
	if c == nil {
		return nil
	}
	counter := &geminiObservedImageCounter{}
	c.Set(geminiImageOutputCounterKey, counter)
	return counter
}

func geminiImageOutputCounterFromContext(c *gin.Context) *geminiObservedImageCounter {
	if c == nil {
		return nil
	}
	...
	counter, _ := value.(*geminiObservedImageCounter)
	return counter
}
