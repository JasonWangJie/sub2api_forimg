...
func (s *GatewayService) ForwardAsChatCompletions(
	...
	if account.Type == AccountTypeAPIKey || account.Type == AccountTypeServiceAccount {
		mappedModel = account.GetMappedModelForRequest(ctx, originalModel)
	}
	...
	// 6. Apply Claude Code mimicry for OAuth accounts or API key accounts that
	// explicitly target a claude_code_only upstream group.
	...
	isClaudeCode := false
	shouldMimicClaudeCode := (account.IsOAuth() || account.IsAnthropicClaudeCodeMimicEnabled()) && !isClaudeCode

	if shouldMimicClaudeCode {
		if account.IsAnthropicClaudeCodeMimicEnabled() {
			anthropicBody = s.applyAnthropicAPIKeyClaudeCodeMimicryToBody(ctx, c, account, anthropicBody, anthropicReq.System)
		} else {
