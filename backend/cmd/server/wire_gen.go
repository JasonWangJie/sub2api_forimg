...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	billingSettingInvalidation := repository.NewBillingSettingInvalidation(redisClient)
	settingService := service.ProvideSettingService(settingRepository, groupRepository, proxyRepository, billingSettingInvalidation, configConfig)
	...
	apiKeyPlatformGroupRepository := repository.NewAPIKeyPlatformGroupRepository(db)
	apiKeyCache := repository.NewAPIKeyCache(redisClient)
	concurrencyCache := repository.ProvideConcurrencyCache(redisClient, configConfig)
	schedulerCache := repository.ProvideSchedulerCache(redisClient, configConfig)
	accountRepository := repository.NewAccountRepository(client, db, schedulerCache)
	concurrencyService := service.ProvideConcurrencyService(concurrencyCache, accountRepository, configConfig)
	apiKeyService := service.ProvideAPIKeyService(apiKeyRepository, userRepository, groupRepository, userSubscriptionRepository, userGroupRateRepository, apiKeyPlatformGroupRepository, apiKeyCache, configConfig, billingCacheService, concurrencyService)
	...
	adminService := service.NewAdminService(userRepository, adminGroupRepository, adminAccountRepository, proxyRepository, apiKeyRepository, apiKeyPlatformGroupRepository, redeemCodeRepository, userGroupRateRepository, userRPMCache, billingCacheService, proxyExitInfoProber, proxyLatencyCache, apiKeyAuthCacheInvalidator, client, settingService, subscriptionService, userSubscriptionRepository, privacyClientFactory, openAIGatewayService, affiliateService, compositeModelRouteRepository, compositeRouteResolver)
	...
	asyncImageTaskCenterHandler := handler.NewAsyncImageTaskCenterHandler(asyncImageTaskService, imageStorageSettingService, userService, apiKeyService, accountService)
	imageDurableStorageService := service.ProvideImageDurableStorageService(imageStorageFactory, imageStorageSettingService, configConfig)
	imageLibraryService := service.ProvideImageLibraryService(imageLibraryRepository, imageStorageSettingService, imageDurableStorageService)
	imageLibraryMaintenanceService := service.ProvideImageLibraryMaintenanceService(imageLibraryService, configConfig)
	imageLibraryHandler := handler.NewImageLibraryHandler(imageLibraryService, imageLibraryMaintenanceService)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, asyncImageTaskCenterHandler, imageLibraryHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, upstreamBillingProbeService, ollamaCloudUsageService)
	...
	asyncImageQueue := repository.NewAsyncImageQueue(redisClient)
	durableAsyncImageHandler := handler.NewDurableAsyncImageHandler(asyncImageTaskService, asyncImageQueue, imageStorageSettingService, secretEncryptor, gatewayHandler, openAIGatewayHandler, apiKeyService, subscriptionService, accountService, imageLibraryService)
	...
	imagePlazaRepository := repository.NewImagePlazaRepository(db)
	imagePlazaService := service.NewImagePlazaService(imagePlazaRepository, configConfig)
	imagePlazaHandler := handler.ProvideImagePlazaHandler(imagePlazaService, imageLibraryService)
	imageWorkbenchService := service.ProvideImageWorkbenchService(apiKeyService, gatewayService)
	imageWorkbenchHandler := handler.NewImageWorkbenchHandler(imageWorkbenchService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageTaskCenterHandler, asyncImageHandler, durableAsyncImageHandler, batchImageHandler, imagePlazaHandler, imageLibraryHandler, imageWorkbenchHandler, idempotencyCoordinator, idempotencyCleanupService)
