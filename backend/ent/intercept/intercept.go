...
// The GroupImageSizeAccountFunc type is an adapter to allow the use of ordinary function as a Querier.
type GroupImageSizeAccountFunc func(context.Context, *ent.GroupImageSizeAccountQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f GroupImageSizeAccountFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.GroupImageSizeAccountQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.GroupImageSizeAccountQuery", q)
}

// The TraverseGroupImageSizeAccount type is an adapter to allow the use of ordinary function as Traverser.
type TraverseGroupImageSizeAccount func(context.Context, *ent.GroupImageSizeAccountQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseGroupImageSizeAccount) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseGroupImageSizeAccount) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.GroupImageSizeAccountQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.GroupImageSizeAccountQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.GroupImageSizeAccountQuery:
		return &query[*ent.GroupImageSizeAccountQuery, predicate.GroupImageSizeAccount, groupimagesizeaccount.OrderOption]{typ: ent.TypeGroupImageSizeAccount, tq: q}, nil
