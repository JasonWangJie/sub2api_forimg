...
更新时间：`2026-08-15`（续更本机专用图片域名 / `local_url` 优先级与文档）。历史 `2026-08-14`、`2026-07-22` 段落仍保留。
...
覆盖：`local_url` 优先于签名下载；填 `local_url` 时新对象可读权限；管理端文案（站点/API vs 静态域名）。

```text
go test ./internal/service -run "LocalImageStorage" -count=1：PASSED
  （含 LocalURLBeatsSignedServe / SignedURLRoundTrip / PublicBaseURL）
文档：wiki-new/对象存储与保留策略.md、生产部署升级与回滚手册.md、部署配置迁移与回滚.md、readmenew.md
生产手工：宝塔网站目录=data_dir + local_url 保存 + 新生成 URL 为 https://图片域名/images/... ：PENDING
```
