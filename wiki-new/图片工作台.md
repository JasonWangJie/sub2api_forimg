...
用户不能手工选择“实时”或“异步”。选择 API Key 后，后端读取该 Key 主分组与可选的 `image_platform_groups` 并返回能力快照；双用 Key 可合并两侧异步 endpoints/models。提交前必须再次获取并比较 `capability_version`。若分组、平台、开关、模型或参数能力发生变化，前端更新表单并停止提交，要求用户重新确认。

下游客户端若只需异步双平台，应优先配置 Key 的 Gemini/OpenAI 生图映射，不必依赖工作台单一主平台展示。详见 [API密钥双平台生图映射.md](API密钥双平台生图映射.md)。

## 固定模式矩阵

| Key 主分组 / 映射 | `execution_mode` | `protocol` | 提交入口 |
|---|---|---|---|
| OpenAI，`allow_async_image_generation=false` | `realtime` | `openai_images` | `/v1/images/generations`、`/v1/images/edits` |
| OpenAI，`allow_async_image_generation=true`（或 openai 映射） | `async` | `openai_async` | `/v1/images/generations_oa`、`/v1/images/edits_oa` |
| Gemini，`allow_async_image_generation=false` | `realtime` | `gemini_native` | `/v1beta/models/{model}:generateContent` |
| Gemini，`allow_async_image_generation=true`（或 gemini 映射） | `async` | `gemini_sc` | `/v1/uploads/images_sc`、`/v1/images/generations_sc` |
| Grok 图片分组 | `realtime` | `grok_images` | `/v1/images/generations`、`/v1/images/edits` |
| Antigravity/其他平台（且无 gemini/openai 映射） | 不可用 | 空 | 不显示为可用生图 Key |
