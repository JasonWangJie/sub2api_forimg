...
接口路径固定，不做配置化。提交响应里的绝对 `query_url` / `Location` 由管理端站点公开地址生成（已保存管理端时以界面为准；否则回落 `async_image.public_base_url`）。
...
1. API Key 有效。
2. 能为该路径解析出计费分组：`image_platform_groups[platform]` 优先，否则主 `group_id` 且平台匹配（详见 [API密钥双平台生图映射.md](API密钥双平台生图映射.md)）。
3. 计费分组平台与路径匹配（`_gm`/`_sc`→gemini，`_oa`→openai）。
4. 计费分组 `allow_image_generation=true`。
5. 计费分组 `allow_async_image_generation=true`。
6. 图片存储配置可用。
7. 现有用户、订阅/余额、Key/账号额度、并发和资格检查通过。
...
- `aspect_ratio=auto/自动` 文生图与图生图均可用；省略上游比例，由模型自选。
...
资产标识使用 `img_*`。用户只能访问自己的资产，越权统一返回 `404`。`view` 默认返回 `307`；客户端发送 `Accept: application/json` 时返回当前 URL 和 `expires_at`。私有图库 `view` 响应为 `Cache-Control: private, no-store`。
...
列表中的 `image_url` 为稳定相对路径 `/api/v1/image-plaza/{publication_id}/content`（路由挂在公开 `v1`，无需登录）。`content` 对本机流式使用 `Cache-Control: public, max-age=3600`；对 OSS 临时重定向使用 `public, max-age≤300`（且短于签名剩余有效期）。异步任务轮询等接口的 `no-store` 不适用于该公开内容入口。
