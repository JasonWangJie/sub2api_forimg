...
branch: main
origin: JasonWangJie/sub2api_forimg
...
- 仓库身份恒为 `JasonWangJie/sub2api_forimg`（不要改回 `Wei-Shaw/sub2api`）
...
7. `backend/migrations/221_ZJ_api_key_platform_groups.sql`（1 Key 双用）。
8. [API密钥双平台生图映射.md](API密钥双平台生图映射.md)。
9. 与当前任务直接相关的 Handler/Service/Repository/前端代码。
10. `docs/DURABLE_ASYNC_IMAGE_API.md`（只针对公共持久异步协议）。
11. 本目录 `文档索引.md`、`当前状态与完成度.md` 和对应专题。
12. 根目录 `readmenew.md` 摘要。
13. 原始需求文档只用于理解意图，不作为最终契约。
...
`2026-07-22` 用户明确要求将功能分支直接非强制合并到 `main`，并要求后续默认直接在 `main` 开发和推送。功能代码主线合并提交为 `a9d23973d`。当时旧 Fork 未启用 Actions，主线交付不能冒充 CI 已通过；该状态不沿用到 `sub2api_forimg`。新仓库通过推送新 tag 触发 Release，并在 Actions 页面核对实际结果；如需单独触发普通 CI，可在 `main` 创建并推送一个明确的空触发提交：
...
```text
这是 JasonWangJie/sub2api_forimg Fork，当前及后续默认在 main 开发和推送。先读 wiki-new/文档索引.md、当前状态与完成度.md、测试与验收记录.md 和 智能助手交接清单.md，再检查脏工作树。Fork 自研 SQL 必须使用 NNN_ZJ_description.sql；182_ZJ 是初版图片广场，185_ZJ 是持久异步任务，186_ZJ 是统一图片对象/图库/审核广场，187_ZJ 是 SC 上传 admission/幂等/恢复，188_ZJ 是本机延期投稿，189_ZJ 是异步结果 PUT intent 与 Outbox claim token，221_ZJ 是 API Key gemini/openai 异步生图计费组映射（1 Key 双用）；上游 182_prompt、183、184 不加 ZJ。异步 Worker 使用 Redis lease token、PostgreSQL CAS/心跳和固定计费命令；持久异步计费组按路径 + image_platform_groups（映射优先，主组回退）解析；工作台模式仍以 Key 能力快照为准，默认私有、公开需审核。发布、安装、升级、在线更新和 GHCR 镜像必须使用 JasonWangJie/sub2api_forimg；上游检查继续使用 Wei-Shaw/sub2api。
```
