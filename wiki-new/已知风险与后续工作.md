...
7. 历史成功异步任务归档回填、永久归档错误终止重排；存储后端切换策略已改为可随时切换（需配合清理）。
...
2. 使用真实 PostgreSQL/testcontainers 验证 `185/186/187/188/189/221`、两阶段 admission、延期投稿状态机、多 Worker、Redis lease token、数据库心跳、Outbox claim token、stale recovery、结果部分上传、引用删除、Key 平台映射解析和旧迁移。
...
6. 功能分支和 `main` 已推送；功能代码主线合并提交为 `a9d23973d`。旧 Fork 当时未启用 Actions；新仓库 `sub2api_forimg` 首次发版时应核对 Release、CI 与 Security Scan 实际结果。
...
当前仍是全站一个存储身份。`2026-08-14` 起后台不再因活跃对象拒绝切换本机/图床/OSS；保存后立即生效。旧对象引用不会自动迁移：未清理就切换可能导致查看失败、清理打到错误后端或留下孤儿文件。生产应先执行管理端「清理全部」或迁移对象；按历史身份解析多套凭证仍是 P1。

保存 OSS Secret / Superbed Token 需要固定 `TOTP_ENCRYPTION_KEY`（或 `totp.encryption_key`）。
...
异步 `worker_concurrency` 按实例累加且没有代码最大值；修改后必须重启，因为 goroutine 数只在启动时创建。Redis Reserve 现在返回独立 lease token，Heartbeat/Ack/Requeue 原子校验所有权；数据库心跳新鲜时不会因 Redis stale delivery 过早标记 `execution_unknown`。server shutdown 已挂接并等待持久异步与图库维护 `Stop()`；真实 PostgreSQL/Redis 多实例下仍需验证网络分区、只有一个上游执行者、租约丢失取消和滚动关闭交接。

### 性能容量不是只调 Worker 数

活动任务约每 15 秒产生 Redis 和 PostgreSQL 心跳，空闲 Worker 约每秒轮询一次 Reserve；实例数和 `worker_concurrency` 会线性放大这些请求。参考图虽已有 8 张/64 MiB/80 MP 默认总预算，但高并发 4K 图仍会同时占用 staging、数据库 WAL、进程内存、OSS 连接和图片并发门禁。生产扩容前应基于 ready/delayed/active、数据库连接池等待、staging 总字节、Outbox/intent oldest age、OSS P95 和账务延迟做压测，不应因为界面已允许任意值就直接设置过大。
...
冲突解决后，按需要重新生成 Ent/Wire，并回归旧同步、旧 Redis 异步、新持久异步（含双用 Key 两侧路径）和站内工作台/图库/广场四类路径。
