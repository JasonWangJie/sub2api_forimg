# 当前状态与完成度

更新时间：`2026-08-14`（续更 1 Key 双用）。历史 `2026-07-22` 基线段落仍保留作归档。

## 版本和仓库

| 项目 | 当前值 |
|---|---|
| 发布版本 | 以 `backend/cmd/server/VERSION` 为准（文档记录时曾见 `0.1.173.2`；历史基线 `0.1.162`） |
| 本轮开发基线 | `51b083d374decf811ac88f8b0194165db9a8ba79`（历史） |
| 基线描述 | `v0.1.162-4-g51b083d37`（历史） |
| 当前及后续默认分支 | `main` |
| 已合并上游 | `upstream/main 5a8d6c4e41e38f05cea4164e6ff03443fc0f6923` |
| 上游合并提交 | `433cf0096` |
| 合并后代码验证提交 | `6412b5eb7` |
| 用户 Fork | `origin = https://github.com/JasonWangJie/sub2api_forimg.git` |
| 原作者仓库 | `upstream = https://github.com/Wei-Shaw/sub2api.git` |
| 初版图片广场迁移 | `backend/migrations/182_ZJ_add_image_plaza.sql` |
| 持久异步迁移 | `backend/migrations/185_ZJ_async_image_tasks.sql` |
| 图库/审核迁移 | `backend/migrations/186_ZJ_image_library_and_plaza_moderation.sql` |
| SC 上传安全迁移 | `backend/migrations/187_ZJ_async_image_upload_reservations.sql` |
| 延期广场投稿迁移 | `backend/migrations/188_ZJ_plaza_submission_deferred_upload.sql` |
| 结果上传/Outbox 并发迁移 | `backend/migrations/189_ZJ_async_image_result_upload_intents.sql` |
| API Key 双平台生图映射 | `backend/migrations/221_ZJ_api_key_platform_groups.sql` |
| 文档记录时 HEAD | `91ecca6114a6e5e8eecac89eccb76ce04603a38b`（工作树可能 dirty） |
| 功能代码主线合并提交 | `a9d23973d352c9923eccdaf789ffd2598d9d0ffe` |
| 合并提交描述 | `v0.1.162-52-ga9d23973d` |
| 功能分支远端 | 已推送 `origin/feat/image-workflow-library-moderation` |
| 功能分支 CI | 旧仓库在 `2026-07-22` 为 `BLOCKED`；新仓库需在首次发版时重新核对 |
| 合并并推送 `origin/main` | `COMPLETED`：用户明确要求直接交付主线，已非强制合并并推送 |

本轮不主动修改 `backend/cmd/server/VERSION`。最终报告不能只写 `0.1.162`，必须同时写完整 SHA、`git describe`、推送目标和 CI 结果。

### Fork 自研迁移命名规则

从本轮开始，Fork 自研 SQL 迁移统一使用 `NNN_ZJ_description.sql`；原作者迁移继续使用原命名，不加 `_ZJ`。当前图片工作流的 `182_add_image_plaza.sql` 以及 `185`、`186`、`187`、`188`、`189` 已完成标记；上游的 `182_prompt_audit_full_prompt.sql`、`183`、`184` 保持原名。

这六个迁移曾以无标记名称部署，因此不能只做文件重命名。迁移器保留严格白名单兼容：旧名记录 checksum 与 `_ZJ` 文件一致时登记新别名且不重复执行，checksum 不一致时拒绝启动。以后新增自研迁移应从首次提交起直接使用 `_ZJ`，不得依赖或扩展本次历史别名机制。

## 工作树中已经存在的实现

| 范围 | 代码状态 | 说明 |
|---|---|---|
| 持久异步任务中心 | 已存在 | 保留上一轮 Gemini BB、OpenAI BB、Gemini SC、PostgreSQL/Redis Worker、固定账单和 OSS 结果 |
| 工作台能力接口 | 已存在 | 按 API Key 所属用户、分组、平台和开关返回 `capability_version`、模式、协议、模型和参数 |
| Key 分组模式矩阵 | 已存在 | OpenAI/Gemini 实时或异步，Grok 仅实时，Antigravity/其他平台不可用 |
| Gemini 实时图片计费采集 | 已存在 | 图片意图、权限/并发门禁、全部 `inlineData`、真实宽高和输出数量 |
| 混合尺寸计费 | 已存在 | 按每张实际规格档位求和，不再按最大规格乘总数量 |
| 服务端个人图库 | 已存在 | OSS 归档、私有默认、游标分页、查看、编辑、软删除、投稿和撤回 |
| 异步结果自动归档 | 已存在且可选 | `async_image.auto_archive_to_library` 默认关闭；开启后成功任务写归档 Outbox，关闭后仍保存结果并返回 URL，不创建个人图库记录 |
| 统一对象引用 | 已存在 | `image_storage_objects` 供异步结果、图库和投稿共享；删除检查活动引用 |
| 审核广场与举报 | 已存在 | `pending_review`、批准、拒绝、下架、恢复、撤回、举报处理和审计事件 |
| 严格图片校验 | 已存在 | PNG/JPEG/WebP 完整容器、实际 MIME、字节/像素限制和常见 polyglot 防护 |
| 旧广场安全迁移 | 已存在 | 先隐藏旧公开记录，再校验、OSS 归档、转私有和待审；危险数据隔离计数 |
| 图库维护 Worker | 已存在 | PostgreSQL 租约/心跳、Outbox、过期清理、对象删除恢复和旧数据迁移 |
| 管理员配置 | 已存在 | 统一 OSS、异步参数、图库保留、条目/容量/单图限制、签名和限频 |
| 用户与管理页面 | 已存在 | 工作台、图库、审核广场、举报、清理/迁移状态和视觉/可访问性调整；广场 `content` 短缓存（2026-08-14） |
| 批量审核 | 已存在 | 静态优先的批量 API、管理端批量批准/拒绝 UI、逐项结果和审计状态转换 |
| 旧删除兼容 | 已存在 | 历史数字 ID、`imgpub_*` 投稿 ID 和 `img_*` 资产 ID 均映射到安全撤回/软删除 |
| 异步归档恢复 | 已存在 | 启动回填升级前成功任务；永久错误终止重排，临时错误才延迟重试 |
| 存储后端切换 | 已调整（2026-08-14） | 可随时切换并立即生效；`backend=local` 支持 `DATA_DIR` 目录回落与可读保存错误；切换前建议管理端 `all`/`async_results` 清理；多凭证并行解析仍为 P1 |
| 管理端存储清理 | 已调整（2026-08-14） | scope 含 `all`、`async_results`、`expired`、`deleted`、`user`；预览可命中异步任务结果 |
| 进程关闭 | 已存在 | server cleanup 同时等待持久异步 Handler 与图库维护 Worker 的 `Stop()` |
| SC 上传 admission | 已存在 | body 前滚动限频，解码/OSS 前幂等与 Key 字节 reservation，PostgreSQL 故障 fail closed |
| SC 上传恢复 | 已存在 | deterministic intent、300/600 秒 Put 超时、至少间隔十分钟二次 Delete、128 alias 上限/墓碑、failed intent 配额和文件名净化 |
| 本机延期投稿（188） | 已存在（可能 dirty） | `image_plaza_submission_requests`；投稿只交元数据；审核通过为 `approved_pending_sync`；用户 sync 才上传 OSS 并 `published` |
| OSS 年月日分区 | 已存在（可能 dirty） | `ImageObjectDatePartition`：`library/.../YYYY/MM/DD/`、`results/YYYY/MM/DD/`、`inputs/YYYY/MM/DD/` |
| 实时本机持图 | 已存在（可能 dirty） | 工作台实时结果默认 IndexedDB；紧凑侧栏不提示异步归档恢复 |
| 异步任务中心 UX | 已存在（可能 dirty） | 任务号加宽+复制；行点击/任务号点击不打开详情，仅「查看」；管理员详情在提示词下展示新任务的可点击参考图 URL |
| 上游失败可观测性 | 已存在（可能 dirty） | `upstream_failed` 的 `error_message` 含 HTTP 状态与脱敏上游正文摘要 |
| Redis Worker 租约所有权 | 已存在（2026-07-22 复核） | 每次 Reserve 独立 token；旧 Worker 不能操作新租约；唯一 `invoking` 可继续 DB 心跳，后处理丢租约则取消 |
| Outbox 认领所有权 | 已存在（2026-07-22 复核） | claim token 防止超时 dispatcher 覆盖新认领结果 |
| 结果上传崩溃恢复 | 已存在（2026-09-07 复核） | 迁移 `189`；PUT 前 intent、日目录下 `yyyyMMddHHmmss+GUID.ext` 确定性 key、部分上传清理 |
| 参考图资源预算 | 已存在（2026-07-22 复核） | 数量、单图/总字节、单图/总像素限制；绑定 OSS 输入直接读取并复验 checksum |
| 并发/恢复性能边界 | 已调整（2026-09-09） | Worker 无代码最大值但修改后需重启；网关生图并发支持系统设置优先、YAML 回退和 Redis 热通知；重试耗尽 SQL 过滤、数据库新鲜心跳防止假 `execution_unknown` |
| API Key 双平台生图映射 | 已存在（2026-08-14） | `221`；映射优先 + 主组回退；submit/Worker/Apply 用解析计费组；Keys/管理端可配；工作台合并双异步入口 |
| 服务器数据库快速迁移工具 | 已存在，待真实演练 | 一小时前 dump 预热、源库冻结、并行最终 data-only、账务摘要和序列校验；禁止仅覆盖余额/Key |

“已存在”只表示代码已经在当前工作树中，不等于已经通过完整测试、CI 或生产验收。

## 本地证据与剩余交付门禁

| 门禁 | 状态 | 完成证据要求 |
|---|---|---|
| 最新 Go 格式化和生成代码 | `PASSED POST-MERGE 2026-07-22` | `gofmt`、`go generate ./cmd/server` 通过且生成代码无差异 |
| 后端全量测试 | `PASSED POST-MERGE 2026-07-22` | Go 1.26.5：交汇定向测试；强制 unit `277.9s`、默认全包 `204.4s`、server build 通过 |
| PostgreSQL 集成与恢复测试 | `PENDING` | `185/186/187/188/189/221`、两阶段 admission、多 Worker、stale lease、Outbox claim、intent/OSS 部分失败、对象引用、双用 Key 解析、迁移幂等 |
| 前端全量门禁 | `PASSED POST-MERGE 2026-07-22` | frozen install、ESLint、typecheck、189 files/1277 tests、974 modules build 通过 |
| 浏览器验收 | `BLOCKED BY TOOLING` | Vite :3000 正常；内置浏览器缺失 sandboxPolicy 元数据，历史 Chrome 10 场景不冒充当前结果 |
| 首页工作台图片资源 | `PASSED 2026-07-22` | WebP `79,374` 字节，首页加载成功 |
| 真实 OSS 厂商契约 | `PENDING` | 七牛、阿里、腾讯逐一 upload、HEAD/read、签名/公开 URL、delete |
| 真实上游与计费核对 | `PENDING` | Gemini/OpenAI/Grok 实时与异步、余额/订阅/倍率/额度逐笔核对 |
| 逻辑拆分提交 | `PASSED` | 主体三笔、`f16c2106a` SC 安全、`81ac080ed` 文档、`433cf0096` 上游合并、`6412b5eb7` 锁文件修复 |
| Fork CI | `PENDING` | 旧仓库历史结果不沿用；在新仓库推送 tag 后核对 Release、CI 与 Security Scan |
| 合并 `origin/main` | `PASSED 2026-07-22` | 用户明确覆盖原 CI 等待顺序；功能代码以 `a9d23973d` 非强制合并，最终远端 SHA 见交付报告或实际 `origin/main` |

### 2026-07-22 异步并发与性能复核增量

本轮基线为 `3602bf6e682e8632c811b97ee50f0d03814c914f`，分支为 `main`。已完成 Redis lease token、Outbox claim token、后处理心跳、重试预算过滤、本地容量延迟重排、参考图总预算/OSS 直读、URL 资源身份 alias、结果 PUT intent 和数据库新鲜心跳保护。当前本地证据：Repository 全包通过；Handler/Repository/Service 三包 `-count=1` 全部通过；Handler `unit` 定向测试通过；`go vet` 通过；前端完整 Vitest、ESLint、类型检查和生产构建通过。最终 SHA 以提交推送后的报告为准。

## 明确未承诺或尚未完成的产品范围

- 视频生成、视频任务或视频广场。
- 实时与异步模式的手工切换或失败互相回退。
- 对 `execution_unknown` 的原任务自动重调。
- 用户取消或删除持久异步任务。
- SC 方言调用 OpenAI。
- 多套 OSS 凭证同时解析历史对象。

## 上线完成定义

只有同时满足以下条件，才可以把状态改为“生产验收完成”：

1. `185`、`186`、`187`、`188`、`189` 在真实 PostgreSQL 备份副本上迁移成功，SC admission/intent、结果部分上传和 Outbox claim 可恢复，延期投稿状态机可恢复，旧公开内容立即隐藏且迁移可恢复。
2. 最新完整后端和前端门禁通过，Fork CI 全绿。
3. 实时/异步四象限、Grok 实时、图库归档失败和异步幂等计费均完成端到端核对。
4. 三家 OSS 的上传、查看、签名过期、对象共享引用和删除完成真实凭证验证。
5. 五种视口及键盘/屏幕阅读器关键路径无阻塞问题。
6. 已拆分提交并推送功能分支，随后非强制合并到 `origin/main`。
7. 本页所有必须门禁的 `PENDING` 已替换为可复核证据。
