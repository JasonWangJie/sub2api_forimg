...
2. **本机目录**：`data_dir` 可留空（开发时常见为 `./data/image_storage` 或 `{DATA_DIR}/data/image_storage`）。本地最简单：留空 `local_url`，把「站点/API 公开地址」或异步 `public_base_url` 填 `http://127.0.0.1:8080`，走签名下载。若要用专用静态域名，把 Nginx 网站目录指到同一 `data_dir` 并填写 `local_url`（见 [对象存储与保留策略.md](对象存储与保留策略.md)）。
