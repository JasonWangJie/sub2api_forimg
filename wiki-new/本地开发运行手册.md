...
`deploy/docker-compose.local.yml` 默认使用当前 Fork 的 `ghcr.io/jasonwangjie/sub2api_forimg:latest`。需要验证尚未发布的本地修改时，应使用 `docker-compose.dev.yml` 从当前工作树构建镜像。
...
进入管理员后台的「数据备份 → 异步生图对象存储」：

1. 选择后端：**本机目录**（本地开发最简单）、**Superbed** 或 **OSS**（`qiniu` / `aliyun` / `tencent` / `custom_s3`）。
2. **本机目录**：`data_dir` 可留空（开发时常见为 `./data/image_storage` 或 `{DATA_DIR}/data/image_storage`）。本地最简单：留空 `local_url`，把「站点/API 公开地址」填 `http://127.0.0.1:8080`（建议「任务查询」公开地址相同），走签名下载。管理端保存后以界面为准。若要用专用静态域名，把 Nginx 网站目录指到同一 `data_dir` 并填写 `local_url`（见 [对象存储与保留策略.md](对象存储与保留策略.md)）。
3. **OSS**：填写 endpoint、region、bucket、access key、secret key 和前缀；本地 MinIO 等通常选 `custom_s3` 并开启 path-style。
4. 执行「测试连接」。测试必须完成上传、读取或 HEAD、删除；失败应显示具体原因（非 `internal error`）。
...
未配置完整存储后端时不要把异步任务失败误判为上游或计费错误。先检查日志中是否出现 `IMAGE_STORAGE_DISABLED`。
