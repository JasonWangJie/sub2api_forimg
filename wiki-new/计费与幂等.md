...
提交时先检查余额、订阅、额度和资格；Worker 调上游前按 **任务 platform** 重新解析计费组（`image_platform_groups` 映射优先，否则主分组回退），须仍等于 `task.group_id`，再检查订阅与开关。如果 Key 被禁用、映射/主组变更导致解析结果变化、普通生图/异步开关关闭或资格失效，应在调用上游前失败且不计费。Apply 阶段按 `task.group_id` 加载完整 Group，避免主分组与计费组不一致时用错倍率。
...
双用 Key 的 `_gm`/`_sc` 与 `_oa` 任务分别使用各自解析出的分组价格；同一 Key 可产生不同 `group_id` 的用量日志。详见 [API密钥双平台生图映射.md](API密钥双平台生图映射.md)。
