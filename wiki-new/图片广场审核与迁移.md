...
| 方法与路径 | 行为 |
|---|---|
| `GET /api/v1/image-plaza` | 只列出已批准、未撤回、未隐藏且未过期的作品；`image_url` 为稳定相对路径 `/api/v1/image-plaza/{id}/content` |
| `GET /api/v1/image-plaza/:publication_id/content` | 公开状态复核后：本机 durable 流式返回，或 `307` 到当前 OSS 签名 URL。浏览器可短缓存（见下） |
| `POST /api/v1/image-plaza/:publication_id/reports` | 登录用户举报公开作品 |

### 广场图片浏览器缓存（2026-08-14）

该 `content` 路由在 `v1` 上**无需登录**即可访问已发布作品，因此对响应开放短缓存；个人图库 `/view` 仍禁止缓存。

| 响应形态 | `Cache-Control` | 说明 |
|---|---|---|
| 本机流式（`writePublishedPlazaImageStream`） | `public, max-age=3600` | 同一相对 URL 内容稳定，网格缩略图与灯箱可复用磁盘缓存 |
| OSS `307` 重定向（`redirectToPublishedPlazaImage`） | `public, max-age≤300`，且短于签名剩余有效期 | 缓存的是稳定入口上的 Location，避免签名过期后仍用旧 Location |
| 用户/管理图库 `.../view` | `private, no-store` | 私有资产，不得共享缓存 |

前端 `/image-plaza` 灯箱与网格使用同一 `resolvePlazaImageUrl`；不在浏览器 IndexedDB 再存一份广场原图。
...
- 自动确保过期清理任务存在，清理按 100 条批次推进；管理员还可创建 `async_results` / `all` 清理任务覆盖异步任务结果与全量存储。
